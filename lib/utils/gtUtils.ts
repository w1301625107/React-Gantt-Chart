...
const cacheParseTime = (function cacheParseTimeInit() {
  const cacheString: { [key: string]: string } = {};
  const cacheValue: { [key: string]: Date } = {};

  return (timeName: string, timeString: string) => {
 ...
 * @returns number
 */
export function getPositonOffset(
  time: string,
 ...
 * @returns
 */
function diffTimeByMinutes(start: Date, end: Date) {
